#!/usr/bin/env python3
"""
sanity_check_fens.py — Quick round-trip evaluation check on a handful of known FENs.

For each FEN the script:
  1. Evaluates with the Python model in fake-quantize mode (matches engine precision).
  2. Evaluates with the Stockfish engine using the same serialized .nnue net.
  3. Asserts the absolute difference is within --max-error internal units.

This catches index-mapping bugs (e.g. a vertically-mirrored king bucket table,
wrong piece-type ordering, wrong orientation sign) *without* needing a dataloader
or thousands of positions.  The test is fast (<5 s) and intended to run before
the full cross_check_eval.py pipeline.

Exit code: 0 = all FENs pass, 1 = one or more FENs fail.
"""
...
# evaluate them).
KNOWN_FENS = [
    # 1. Starting position — symmetric, eval must be near 0; catches sign / offset bugs.
    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",

    # 2. Open Ruy-López, white king still on e1 (rank-1 region, middle files).
    ...
    # 5. Black to move — exercises the side-to-move perspective flip.
    "r1bqkb1r/pppp1ppp/2n2n2/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R b KQkq - 4 4",

    # 6. White king on h8-area (rank-8 region) — catches vertical-flip of bucket table.
    "6k1/8/8/8/8/8/8/R5K1 w - - 0 1",

    # 7. KQ vs K — exercises PSQT columns and a different material balance.
    "8/8/4k3/8/8/4K3/8/7Q w - - 0 1",

    # 8. Pawn-heavy middlegame — exercises many pawn features.
    "r1bqkbnr/pp1ppppp/2n5/2p5/4P3/5N2/PPPP1PPP/RNBQKB1R w KQkq - 2 3",
]


# Quantization noise between fake-quantized Python eval and engine eval is
# typically 1-2 internal units.  A threshold of 2 units is tight enough to
# catch any real index-mapping bug (which causes errors on the order of 50-200
# units) while still tolerating normal quantization rounding.
DEFAULT_MAX_ERROR = 2
...
def main() -> None:
    ...
    # Filter positions that cannot be evaluated by the engine (king in check)
    fens = [f for f in KNOWN_FENS if not chess.Board(f).is_check()]

    W = 72
    print(f"\n{'='*W}")
    print(f"  FEN Sanity Check  -  {len(fens)} positions  "
