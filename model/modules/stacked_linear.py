...
class StackedLinear(nn.Module):
    ...
    def _init_uniformly(self) -> None:
        ...
        if self.has_bias and self.linear.bias is not None:
    ...
    def at_index(self, index: int) -> nn.Linear:
        layer = nn.Linear(self.in_features, self.out_features, bias=self.has_bias)
...
class FactorizedStackedLinear(StackedLinear):
    def __init__(
        ...
        super().__init__(in_features, out_features, count, quantization, layer_key, bias=bias)

        self.factorized_linear = nn.Linear(in_features, out_features, bias=bias)
